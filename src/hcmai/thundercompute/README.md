...
`hcmai.thundercompute` is the boundary between the local HCMAI search
...
[deployment guide](../../../scripts/thundercompute/README.md). This document describes the Python
...
PYTHONPATH=src aic/bin/python -m uvicorn hcmai.thundercompute.server.api:app \
...
Code outside `hcmai.thundercompute` imports only `LLMService` and configuration
contracts from `hcmai.thundercompute.pipeline`. It must not import `server/` or
...
python -m compileall -q src/hcmai/thundercompute
```
