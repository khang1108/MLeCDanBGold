...
class OCRBackend(Protocol):
    """Backend contract used by the resumable OCR pipeline."""
...
class OCREngine:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            self.resolved_revision = (
                getattr(self.model.config, "_commit_hash", None) or self.config.revision
            )
    ...
    def recognize_batch(self, images: Sequence[Image.Image]) -> list[OCRResult]:
        ...
        inputs = self.processor(
            text=["<OCR>"] * len(images), images=list(images), return_tensors="pt", padding=True
        )
        dtype = {"bfloat16": torch.bfloat16}.get(self.config.dtype, torch.float32)
        inputs = {
            key: (
                value.to(self.config.device, dtype=dtype)
...
def _valid(data: FrameRow, config: OCRConfig) -> FrameEnrichment | None:
    try:
...
def _parsed(
    ...
    if confidence is not None and (
        not isinstance(confidence, (int, float)) or not math.isfinite(float(confidence))
    ):
    ...
    evidence = {
        "frame_id": frame_id,
        "raw_output": str(result.raw_output)[:500] if result.raw_output is not None else None,
    ...
    for start in range(0, len(todo), config.batch_size):
        ...
        for frame in todo[start : start + config.batch_size]:
            try:
                ...
                rows[frame["frame_id"]], failures[frame["frame_id"]] = _failure(
                    frame["frame_id"], config, "image_load", error
                )
        if not valid:
            continue
        results: list[object]
...
def _report(
    ...
    summary = (
        {"min": min(confidence), "max": max(confidence), "mean": sum(confidence) / len(confidence)}
        if confidence
        else None
    )
    return {
        ...
        "frames_with_text": text,
        "empty_text_frames": complete - text,
        "failed_frames": len(rows) - complete,
        "skipped_frames": skipped,
        "retried_frames": retried,
        "disabled_frames": disabled,
        "text_coverage_rate": ratio(text),
        "empty_text_rate": ratio(complete - text),
...
def generate_ocr(
    ...
    *,
    ...
    prior_evidence = old.get("raw_evidence", [])
    evidence: dict[str, Evidence] = {
