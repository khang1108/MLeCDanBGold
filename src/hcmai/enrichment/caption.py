...
class CaptionConfig:
    ...
    image_size: int = 768
    batch_size: int = 4
    enrichment_version: str = "caption_v1"
    write_interval: int = 25
    ...
    def from_dict(cls, data: dict[str, Any]) -> CaptionConfig:
        ...
        values = dict(data)
        values["model_checkpoint"] = values.pop("name", values.get("model_checkpoint", cls.model_checkpoint))
        if "precision" in values and "dtype" not in values:
            values["dtype"] = {"fp16": "float16", "bf16": "bfloat16"}.get(values["precision"], "float32")
        config = cls(**{key: value for key, value in values.items() if key in cls.__dataclass_fields__})
        if min(config.batch_size, config.image_size, config.write_interval) < 1:
...
class FrameCaptioner:
    ...
    def __init__(self, config: CaptionConfig, model: Any = None, processor: Any = None,
        ...
        self.batch_fn, self.resolved_revision = batch_fn, config.revision
    ...
    def _load(self) -> None:
        ...
        types = {"float16": torch.float16, "fp16": torch.float16, "bfloat16": torch.bfloat16, "bf16": torch.bfloat16}
...
def _resume(frames: list[dict[str, Any]], path: Path, config: CaptionConfig):
    ...
    if path.exists():
        for data in pd.read_parquet(path).to_dict(orient="records"):
    ...
    for frame in frames:
        ...
        if row:
            ...
            continue
        retried += bool(old)
        rows[frame_id] = FrameEnrichment(frame_id=frame_id, model_name=config.model_checkpoint, enrichment_version=config.enrichment_version, status=ProcessingStatus.PENDING)
        todo.append(frame)
...
def _write(output: Path, order: list[str], rows: dict[str, FrameEnrichment], failures: dict[str, dict[str, str]]) -> None:
    data = [rows[key].model_dump(mode="json") for key in order]
    write_parquet(pd.DataFrame(data), output / "frame_enrichment.parquet", index=False)
...
def _run(todo: list[dict[str, Any]], order: list[str], rows: dict[str, Any], failures: dict[str, Any],
         captioner: FrameCaptioner, config: CaptionConfig, output: Path) -> list[float]:
        ...
        if valid:
            ...
            for (frame_id, _), result in zip(valid, results):
                if isinstance(result, Exception) or result is None or not str(result).strip():
                    error = result if isinstance(result, Exception) else ValueError("empty caption")
                    rows[frame_id], failures[frame_id] = _failure(frame_id, config, "model", error)
                else:
...
def _git_commit() -> str | None:
    try:
        ...
        return None
def _manifest(config: CaptionConfig, frames_path: Path, rows: dict[str, Any], captioner: FrameCaptioner,
    ...
    return {"artifact_version": "frame_enrichment.v1", "enrichment_version": config.enrichment_version,
            "dataset_version": config.dataset_version, "input_parquet_path": str(frames_path),
...
def generate_captions(frames_path: str | Path, output_dir: str | Path, config: CaptionConfig,
                      captioner: FrameCaptioner | None = None) -> dict[str, Any]:
    ...
    frames = pd.read_parquet(frames_path).to_dict(orient="records")
    ...
    latencies = _run(todo, order, rows, failures, captioner, config, output)
    _write(output, order, rows, failures)
    manifest = _manifest(config, frames_path, rows, captioner, old, started, perf_counter() - began, latencies, skipped, retried)
...
def main() -> int:
    ...
    raw = read_yaml(args.config) or {}
    values = raw.get("caption", raw.get("models", {}).get("caption", {}))
    values = {**values, "dataset_version": raw.get("dataset", {}).get("version", values.get("dataset_version", "unknown"))}
