...
def test_build_is_canonical_and_store_resolves_official_mapping(
    ...
    record = data.get_frame("L21_V001_keyframe_000001")
    assert (record.video_id, record.frame_idx) == ("L21_V001", 90)
    assert [row.frame_id for row in data.iter_frames()] == (
        first["frame_id"].tolist()
    )
    assert data.contains_submission("L21_V001", 90)
    assert not data.contains_submission("L21_V001", 1)
