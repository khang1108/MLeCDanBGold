...
def align_video_conditioned(
    ...
    *,
    ...
    for event in range(n_events - 2, -1, -1):
        ...
        # a position where starts[succ] >= t + 1. When cluster_delta == 0, starts[k] = k,
        # so this simplifies to succ >= t + 1 (strict chronological order).
        # When cluster_delta > 0, searchsorted jumps past all frames in t's cluster to
        # the first frame of the next cluster, maintaining exact symmetry with forward DP.
        next_valid_start = np.searchsorted(starts, frames + 1, side="left")
