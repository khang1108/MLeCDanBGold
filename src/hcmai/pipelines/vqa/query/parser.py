...
from ..domain.models import ParsedVQAQuery, QuestionType
...
_RULES: tuple[tuple[QuestionType, tuple[str, ...]], ...] = (
    ...
    (
        QuestionType.TEXT,
        ("what does", "written", "read", "chữ gì", "ghi gì", "biển"),
    ),
    (
        QuestionType.SPEECH,
        ("say", "said", "tell", "nói gì", "đọc gì", "hỏi gì"),
    ),
    (
        QuestionType.TEMPORAL,
        ("before", "after", "then", "trước", "sau", "tiếp theo", "vì sao"),
    ),
    (
        QuestionType.IDENTITY,
        ("who", "what is", "which object", "ai ", "là gì", "vật gì"),
    ),
)
...
def parse_vqa_query(request: VQARequest) -> ParsedVQAQuery:
    ...
    return ParsedVQAQuery(
        ...
        # The question is answer intent, never a first-stage localization hint.
        clue_queries=(),
    )


def _clean(value: str) -> str:
    """Normalize user text while preserving its original language and meaning."""

    return re.sub(r"\s+", " ", unicodedata.normalize("NFC", value)).strip()


def _detect_language(value: str) -> QueryLanguage:
    """Detect the supported answer language with conservative lexical rules."""
...
def _modalities(kind: QuestionType) -> tuple[RetrievalSource, ...]:
    """Return evidence modalities preferred after scene localization."""
