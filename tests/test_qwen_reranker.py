...
def test_initialization_failure_is_cached(monkeypatch, error):
    ...
    value = QwenRerankerScorer(QwenRerankerConfig())
    for _ in range(2):
...
def test_composition_preserves_candidates(tmp_path):
    ...
    store = SimpleNamespace(get=lambda frame_id: records[frame_id])
    output = MultimodalReranker(
        store, RerankerConfig(batch_size=2), scorer().score_batch).rerank("q", candidates)
