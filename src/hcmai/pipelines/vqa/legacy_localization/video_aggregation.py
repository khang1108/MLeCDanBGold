"""Fallback frame-evidence aggregation before VQA inference."""

from __future__ import annotations

from collections import defaultdict

from hcmai.common.schemas import FrameEvidence, RetrievalSource

from ..domain.models import VideoEvidenceCandidate


def aggregate_videos(
    candidates: list[FrameEvidence], *, top_videos: int = 10, neighborhood_ms: int = 15_000
    ...
    groups: dict[str, list[FrameEvidence]] = defaultdict(list)
...
def _best_rank(frames: list[FrameEvidence], unit_id: str) -> int | None:
    ranks = [index for index, frame in enumerate(frames, 1) if unit_id in frame.unit_scores]
