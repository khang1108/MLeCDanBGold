...
The builder follows the existing AIC fixture layout:
...
It also accepts the shorter `map-keyframes/*.csv` plus
`keyframes/<video_id>/*.jpg` layout already supported by the repository.
Mapping CSVs must contain `n`, `pts_time`, and official `frame_idx`.
The numeric image stem matches `n`.
