"""Native Qwen3-VL relevance adapter for ordered query-image pairs."""
...
class QwenAdapter:
    ...
    def score(self, query: str, images: Sequence[Image.Image]) -> list[float]:
        ...
        try:
            ...
            raise QwenRerankerError(
                f"Qwen scoring failed: {_bounded(error)}"
            ) from error

    def score_batch(
        self, query: str, images: Sequence[Image.Image]
    ) -> list[float]:
        """Hosted-inference compatibility; delegates to the adapter contract."""
        return self.score(query, images)
    ...
    def metadata(self) -> dict[str, Any]:
        return {
            ...
            "processor_class": (
                type(self.processor).__name__ if self.processor else None
            ),
