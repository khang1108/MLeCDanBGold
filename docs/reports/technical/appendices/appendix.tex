...
\toprule
Presented subsystem & Appendix evidence & Technique / method boundary \\
\midrule
End-to-end architecture / services & Fig.~\ref{fig:architecture}; Table~\ref{tab:models} & Orchestration, task routing, and model coordination \\
Keyframe ingestion / identity & Fig.~\ref{fig:preparation}; Table~\ref{tab:parameters} & C++/FFmpeg 1-FPS sampling, canonical identity schema \\
Caption, OCR, objects, ASR, FrameContext & Fig.~\ref{fig:preparation}; Tables~\ref{tab:models}--\ref{tab:artifacts} & Specialist enrichment and deterministic context serialization \\
FAISS / BM25 / fusion & Figs.~\ref{fig:preparation} and~\ref{fig:temporal}; Tables~\ref{tab:artifacts}--\ref{tab:parameters} & Dense/sparse indexing, RRF and adaptive evidence fusion \\
Temporal DP / KIS / TRAKE & Figs.~\ref{fig:temporal} and~\ref{fig:dp-explainer} & Prefix-max monotonic DP, calibration, cue routing \\
KIS operator flow & Fig.~\ref{fig:ui-kis} & Natural-language planner, representative frame selection \\
TRAKE operator flow & Fig.~\ref{fig:ui-trake} & Ordered-event input, complete same-video path submission \\
Video preview / evidence inspection & Fig.~\ref{fig:ui-player} & Frame-accurate seek, specialist evidence overlay \\
Collaborative workspace / submissions & Fig.~\ref{fig:ui-workspace} & Revision-aware shared answer files, query history replay \\
Thunder Compute / S3 execution & Fig.~\ref{fig:architecture}; text below & Ephemeral GPU build, versioned artifact publication \\
\bottomrule
\end{tabularx}
\end{table}
